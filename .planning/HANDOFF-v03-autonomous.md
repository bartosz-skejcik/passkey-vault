...
2. **Wave 2 ZMERGOWANY** — `.github/workflows/ci.yml` + web `compile` na main; pełny 4-job gate zielony lokalnie (rust workspace, web 481, ext 693 + oba buildy + web-ext lint + MAIN-world audit, supply-chain ok). Tracking zrobiony. **WSZYSTKIE 4 plany fazy 20 gotowe.** START TU: krok od `## KROK 5` niżej (review → verify → transition → lifecycle). Prompt jak w tej sesji (worktree_branch_check z nową bazą). Plan sam robi lokalny full-gate „green vs main" (brak git remote → lokalny przebieg to osiągalny dowód, cloud-run flagowany jako follow-up).
