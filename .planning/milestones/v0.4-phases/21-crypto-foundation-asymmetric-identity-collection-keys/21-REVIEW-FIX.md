...
- All three `pv-wasm` call sites (`decryptItem`, `decryptItemForCollection`,
  ...
  **CORRECTION (added by the orchestrator after the iteration-3 review, which disproved the
  original claim here).** This bullet originally asserted the intermediate plaintext buffer is
  "wiped on every path (including the `String::from_utf8` failure path)". That is **false as
  written** and must not be relied on. `mem::take` moves the buffer *out* of `Zeroizing`'s
  protection, so at all three sites nothing is wiped on either path — on the error path
  `FromUtf8Error` owns the bytes and is dropped intact. Net exposure is unchanged from pre-fix
  (not worse, but not improved either). Tracked as **WR-14** in `21-REVIEW.md`, assessed
  non-blocking, with a stated 3-line remedy: validate by borrow first, then take.
