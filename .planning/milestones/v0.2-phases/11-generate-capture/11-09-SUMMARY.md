...
## Addendum 2 (2026-07-16, same-day correction): unlock-view regression + "one merged scroll container" clarification

Two issues surfaced against the Addendum-1 fix (`a08e9b2`) within the same review round, both fixed together in commit `62a3df4`, `fix(11-09): per-view popup height + single merged section-scroll`:

**Regression (Bartek screenshot, unlock view):** Addendum 1 pinned `body { height: 600px }` globally in `index.html`, plus `#root { height: 100% }` in `style.css`, so that fixed height cascaded down into `h-full` on EVERY view's root — not just the item list. UnlockView, ServerConfigView, ItemDetailView, and the loading spinner all got forced to a constant 600px, producing a large empty gap under short content (Bartek's screenshot: unlock view with a wide blank area below "Change server"). Before Addendum 1, Chrome auto-sized the popup window to each view's natural content height; that behavior was lost for every view except the one it was actually needed for.

**Fix:** moved the fixed height off `body`/`#root` entirely. `index.html`'s `body` keeps only `overflow: hidden` (no explicit height); `style.css` dropped `#root { height: 100% }` (dead now that its parent has no definite height to resolve a percentage against). The `h-[600px]` hard pin now lives locally on the ONE wrapper that actually needs it — App.tsx's item-list-view branch. Every other view (`UnlockView`, `ServerConfigView`, `ItemDetailView`, App.tsx's loading state) is back to natural/auto height, with `max-h-[600px] overflow-y-auto` on the three form/detail views as a harmless ceiling+single-scroll-fallback for the rare case their content would otherwise exceed the popup's own height cap.

**Further clarification (Bartek, same round):** "Nie powinno być 2 scrolli pod sekcjami. Powinien być jeden kontener scrollowalny w środku z tekstami od sekcji i jedna sekcja pod drugą z itemkami." Addendum 1's shape still had TWO scroll boxes inside the list view: `OnThisPageSection`'s own bounded `max-h-[140px] overflow-y-auto` match list, and the separate `flex-1 min-h-0 overflow-y-auto` "Wszystkie" box below it — each independently scrollable, which is exactly the "2 scrolli pod sekcjami" pattern Bartek was rejecting, just scoped one level deeper than the original outer-page bug. Fixed by removing `OnThisPageSection`'s internal scroll bound entirely and merging both sections into ONE shared `flex-1 min-h-0 overflow-y-auto` container in `ItemListView.tsx`: "Na tej stronie" (heading + rows/hint) and "Wszystkie" (heading + rows) are now plain section-under-section siblings inside that single container, their headings scrolling away with their own content (deliberately not sticky). The restricted/unreachable pageState's static warning banner is the one exception kept pinned outside the scroll region (alongside the top bar/search/footer), since it's not scrollable list content.

**Files touched:** `extension/entrypoints/popup/index.html`, `extension/entrypoints/popup/style.css`, `extension/entrypoints/popup/App.tsx`, `extension/entrypoints/popup/ItemListView.tsx`, `extension/entrypoints/popup/ServerConfigView.tsx`, `extension/entrypoints/popup/UnlockView.tsx`, `extension/entrypoints/popup/ItemDetailView.tsx`, `extension/entrypoints/popup/autofill/OnThisPageSection.tsx`.

Presentation-only — 401/401 vitest tests, `tsc --noEmit`, and both `wxt build` targets (chrome-mv3 + firefox-mv2) green; packaged `popup.html`/`popup.css` spot-checked to confirm `body` no longer carries an explicit height and `#root{height:100%}` no longer ships.

**Lesson (compounding the one above):** a fix for "the outer popup scrolls" must not blanket-apply its constraint (fixed height, hidden overflow) to every view sharing that document — only the view whose layout actually requires it. And a "single scroll region" requirement should be checked recursively: fixing the outermost double-scroll can still leave a nested double-scroll one level deeper (section-vs-section) if the same hand-guessed bounded-height pattern (`max-h`/`overflow-y-auto`) was applied to more than one region.
