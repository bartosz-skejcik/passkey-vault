...
Closes the 1 Critical + 8 Warnings from `09-REVIEW.md` and gap 1 from
`09-VERIFICATION.md`. Every fix carries a regression test **verified failing
against the pre-fix code** — this phase had 9 UAT-found bugs that green unit
suites missed, so a fix without a genuine RED is not a fix.

**Final counts:** extension **143** (was 141; +17 new, −15 removed with the dead
surface), cargo **129** (was 118; +11), web **345** (unchanged — untouched).
`tsc --noEmit` clean; `wxt build -b chrome` and `-b firefox` both green.

| # | Finding | Fix | Regression test | Commit |
|---|---------|-----|-----------------|--------|
| **CR-01** | Popup kept rendering a decrypted (possibly revealed) password after auto-lock — `ItemListView` held the only `vault.updated` listener and is unmounted on `ItemDetailView`, so the background's T-09-18/19 lock hardening was defeated one layer up | `App.tsx` (the one component mounted for *every* view) owns a top-level `session.locked` listener; on lock it re-reads authoritative `session.status` and resets the view from **any** view, unmounting `ItemDetailView` and dropping the plaintext from React state. New dedicated `session.locked` broadcast from `lockVaultSession()` — deliberately **not** overloading `vault.updated`, which also fires on every ordinary sync merge (would cost a `session.status` round trip per merge, IN-02) | `App.test.tsx`: render on the detail view, fire `session.locked`, assert the decrypted fields are gone from the DOM and UnlockView is shown | `6c1dff3` |
| **WR-01** | `void handle(m).then(sendResponse)` had no rejection path → a throwing handler never called `sendResponse`, hanging the popup + leaking an unhandled rejection | Added a rejection handler returning a typed `{ok:false,error:"unknown"}`. Also hardened `ensureHydrated()`: a corrupt/un-importable envelope now resolves `null` (treated as locked, envelope cleared) instead of throwing — that was the likeliest real rejection, and it stranded the popup on the loading spinner forever | `router.test.ts`: a rejecting handler still resolves `sendResponse`. `vault-session.test.ts`: a corrupt envelope resolves `null` and clears itself | `5ac20bc` |
| **WR-02** | Inverted error taxonomy in `handleExtEnrollFinish`: `ApiClientError`→`"unknown"`, everything else→`"unreachable"` (telling users their server was down when it answered); no 401 branch | Matches `unlock.ts`'s taxonomy: 401→`invalid-credentials`, other `ApiClientError`→`unknown`, non-API→`unreachable`. `"invalid-credentials"` added to the response union | `ext-passkey.test.ts`: a 500, a 401, and a network failure each map to the right error | `ef0cdac` |
| **WR-03** | `handleExtPrfUnlockFinish` was `try`/`finally` with **no catch** — a short/malformed `prfB64` (which crosses the popup boundary as base64 that `b64ToBytes` decodes to any length) threw out with no typed result, hanging the ext-PRF unlock | Added a catch mirroring `handleUnlockPrfFinish` | `ext-passkey.test.ts`: a throwing `fromExtPrf` and a rejecting `setUnlockedUserKey` both resolve typed failures; the `finally` zeroize still runs | `ef0cdac` |
| **WR-04** | Doc comments claimed a stronger zeroize invariant than holds — the base64 hop leaves **immutable** JS strings with the raw UK bytes that `lockVaultSession()` cannot clear | **Comment only, no code change** (as scoped). `bytesToBase64`/`base64ToBytes`/`setUnlockedUserKey`/`ensureHydrated` now state the real bound at the point of risk: the `finally { fill(0) }` covers the `Uint8Array` **only**; the strings are immutable, un-wipeable, not cleared by a lock, and bounded to the SW heap (never reachable by a page, content script, `storage.local`, a log, or the server) — mirroring `UnlockView.tsx`'s existing wording for its own b64 exposure | n/a — documentation correction | `a1ce563` |
| **WR-05** | `setUnlockedUserKey` never armed the alarm; arming depended on `App.tsx`'s incidental follow-up `session.status`, so EXT-03 rested on a coincidence | Arms inside `setUnlockedUserKey`, at the actual state transition, with the interval it is itself writing. `session.setAutoLockMinutes` stays the sole authority for interval changes and does not double-arm (the `noteActivity` race fix at `router.ts:72` is untouched) | `vault-session.test.ts`: unlocking arms an alarm with **no** `session.status` call; arms exactly once. `router.test.ts`'s 4 existing assertions still green | `a1ce563` |
| **WR-06** | The poll fallback was a `setInterval` — destroyed by an MV3 idle-kill, i.e. dead in exactly the WS-stripped-proxy scenario it exists for (it only worked when the WS was healthy, i.e. when it wasn't needed) | `chrome.alarms`-backed under the **distinct** name `pv-sync-poll` (a collision with `pv-auto-lock` would silently replace the auto-lock alarm and disable EXT-03). Listener registered synchronously at startup. Period is an honest 1 min: Chrome clamps `periodInMinutes` to ≥1 in release builds, so the nominal 30s was never honored | `sync-client.test.ts` (4): the alarm is created under its own name and never collides; its handler triggers a pull with no WS activity; an unrelated alarm doesn't; `stopSync` clears it and a late fire never repopulates a locked vault | `f3374fc` |
| **WR-07** | `PV_EXTENSION_ORIGINS` silently dropped malformed entries (collapsing the allowlist to "no CORS", indistinguishable from unset, with nothing in the log) and **panicked** the server at boot on `*` | Both now fail loudly at startup naming the offending value, via `Config::validate()` (this project's fail-loud convention / DEPLOY-02, already called from `main.rs` before bind) — checked *before* the localhost early-return, since `*` would panic on any deployment. `build_cors_layer` keeps a log-and-degrade path so it can never panic | 10 Rust tests: wildcard + malformed each fail `validate()` by name on both localhost and non-localhost; unset/valid pass; `build_cors_layer` is panic-free and fails **closed** on both. Plus `upstream_allow_origin_list_still_panics_on_a_wildcard`, a `#[should_panic]` test pinning the exact upstream behavior the fix routes around | `1ba1055` |
| **WR-08** | Dead surface still shipping | Removed **689 net lines**: (a) `spike.roundtrip` + `lib/crypto/vault-session.ts`'s hard-coded-`SPIKE_PASSWORD` Argon2 path + its listener — its own comment said it should already be gone, yet a debug path with a hard-coded credential shipped to users; (b) the entire web-RP PRF pair (4 message kinds, 4 handlers, router cases, `auth-api` transport) — a `chrome-extension://` page gets `SecurityError` for any web RP ID, so it was unreachable **by construction**, and the router cases widened `isProtocolMessage`'s surface around handlers no shipped code exercised; (c) `buildPrfExtensions`/`stripPrfFromCredentialJson`/`deleteExtensionPasskey`. **Kept** `extractPrfBytes` (the ext-PRF unlock + enroll paths use it). Why the web-RP pair can never come back is documented in `ext-protocol.ts`'s header | `ext-protocol.test.ts`'s fixtures are typed `{[K in Message["kind"]]: ...}`, so the shrunken maps are **tsc-enforced** proof the union really shrank; `unlock.test.ts`'s tests for the removed handlers deleted | `9a18594` |
| **EXT-05 gap** | ROADMAP SC #1 requires the server URL be "persisted **and editable later**" — there was no UI path back to `ServerConfigView` once a config existed, so a typo'd/moved server was unfixable without wiping storage. (09-CONTEXT deferred this as scope creep; a CONTEXT deferral cannot subtract a roadmap SC, and no later phase covers it) | Discreet muted text link on the **unlock** view — where a stuck user actually is. *Not* the list footer (cramped), *not* the header gear (redirects to the web app's settings per EXT-06's binding NordPass layout; could never change the extension's own `baseUrl`). `ServerConfigView` gains optional `initialUrl` + `onCancel` → two modes: first-run gate (no seed, no cancel, still blocking) and reconfigure (pre-filled, cancel returns to unlock). **Identical validation path** for both (normalize → `config.set` probes `/healthz` → persist); persist-before-permission-prompt order untouched. Copy: PL "Zmień serwer" / EN "Change server" | `App.test.tsx` (4): the link renders on unlock; opens the config view pre-filled with the current URL; cancel returns to unlock having dispatched no `config.set`; a successful change dispatches `config.set` with the new URL and leaves the view | `a9e63ca` |

### Not fixed, deliberately

- **The 6 Info findings (IN-01…IN-06)** — out of scope per the orchestrator's instruction ("do NOT address unless trivially free"). None were trivially free: each needs its own regression test and IN-04/IN-06 are cross-cutting. **IN-02** (double `vault.updated` broadcast per snapshot) is worth noting: the review flagged that CR-01's fix would make it worse if the lock listener hung a `session.status` off `vault.updated` — the dedicated `session.locked` broadcast sidesteps that entirely, so IN-02 stays a pure efficiency nit rather than becoming a CR-01 tax. **IN-06** (clipboard never cleared) is a real carried-over gap from v0.1 and deserves a cross-cutting phase item.
- **D-02's raw-UK-bytes WASM→JS crossing** — a documented, accepted decision; explicitly out of scope. WR-04 documents its *residue* honestly without reopening the decision.
- **The auto-lock persist/race fix** (`router.ts:212-233`, `:72`) — verified working in a real browser and pinned by `router.test.ts`; untouched. WR-05 was implemented specifically so as not to disturb it.
