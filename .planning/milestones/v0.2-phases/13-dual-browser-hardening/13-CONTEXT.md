...
## ADDENDUM (2026-07-17) — Bartek decisions after the freshness audit (13-FRESHNESS-AUDIT.md)

Asked via AskUserQuestion after the 5x Opus audit; these OVERRIDE any conflicting plan text and the audit's own recommendations where they differ.

- **D-10 (moz-extension CORS mechanism — Bartek, overrides audit's concrete-origins-only recommendation):** pv-server gains FIRST-CLASS support for the literal entry `moz-extension://*` in `PV_EXTENSION_ORIGINS` — a dedicated, scheme-scoped wildcard implemented via a deliberate mechanism (e.g. `AllowOrigin::predicate` matching well-formed `moz-extension://<uuid>` origins), NEVER by loosening the bare-`*` rejection (WR-07 stays: bare `*` and any other wildcard remain fatal at startup). **FLAGGED AS TECH-DEBT by Bartek's own instruction ("oflaguj to jako do zmiany później")**: record in STATE.md Deferred Items + SELF-HOSTING.md that the scheme-wildcard is a stopgap to be replaced with per-install concrete-origin configuration in a later version. Rationale for accepting the risk now: CORS is not an auth boundary (API requires bearer tokens; a hostile FF extension without a token gains nothing), and Firefox per-profile UUID churn makes concrete origins hostile UX today.
- **D-11 (CORS-blocked vs unreachable):** ServerConfigView distinguishes a CORS-blocked `/healthz` probe from a genuinely-unreachable server. CORS-blocked shows a specific, actionable message: server reached but rejected this extension's origin — include the extension's OWN origin as copyable text + pointer to `PV_EXTENSION_ORIGINS` docs. Implementation hint (executor discretion): on fetch TypeError, retry `{mode:'no-cors'}` — opaque success ⇒ reachable-but-CORS-blocked; second failure ⇒ unreachable.
- **D-12 (Firefox ext-scoped passkey degradation presentation — Bartek, overrides audit's hide-recommendation):** if the extension-scoped unlock passkey (rpId = extension id) is unusable on Firefox, the passkey affordance stays VISIBLE but DISABLED with an inline explainer (both UnlockView unlock button and EnrollExtPasskeyPrompt enroll offer). Password unlock always fully functional alongside (D-06). Never a silent dead-end.
- **D-13 (D-03 copy canon — Bartek):** the UNLOCK/ENROLL popup surface adopts the 13-UI-SPEC wording (browser-framed: PL "Szybkie odblokowanie passkeyem nie jest dostępne w tej przeglądarce — użyj hasła." / EN "Fast unlock isn't available for this passkey on this browser — use your password."), replacing/aligning `unlock.passkeyUnsupported` and the new dead-end banners. The PROVIDER ceremony copy stays the shipped WR-02 site-framed `provider.prfUnavailableNote` — do NOT reintroduce browser-blaming there (D-16 phase 12: provider PRF is WASM-computed, browser-independent).

**Structural consequence:** NEW plan 13-05 (moz-CORS: server `moz-extension://*` mechanism + tests + docs + ServerConfigView D-11 UX), ROADMAP §13 grows to 5 plans; 13-04 depends_on [13-01, 13-03, 13-05].
