...
| Property | Value |
|----------|-------|
| **Framework** | Vitest 3.2.4 (extension unit tests, new to `extension/` this phase) + Playwright `@playwright/test` (Chromium E2E, new to `extension/` this phase) + `web-ext lint`/`web-ext run` (Mozilla CLI, static analysis + Firefox launch) |
| **Config file** | `extension/vitest.config.ts` (new, Plan 13-02), `extension/playwright.config.ts` (new, Plan 13-03) |
...
- **Before `/gsd-verify-work`:** All 21 `13-UAT-CHECKLIST.md` rows green (`PASS` or `RESOLVED`), `web-ext lint` clean on the packaged Firefox build, Chrome Playwright suite green
...
| 13-03-01 | 03 | 2 | XBR-01 | T-13-06/T-13-SC2 | Chromium extension-testing harness covers all 19 SCs; virtual authenticator confined to test project | static (test count) | `node -e "..." test count check` | ❌ Wave 0 | ⬜ pending |
| 13-03-02 | 03 | 2 | XBR-01 | T-13-07 | Full Chrome suite run; 21-row checklist authored with Chrome results | integration | `npm run test:e2e:chrome -- --reporter=list` (exit code asserts pass/fail, no output-masking pipe) | ❌ Wave 0 | ⬜ pending |
| 13-04-01 | 04 | 3 | XBR-01 | T-13-09/T-13-10 | Firefox walk complete; D-05/D-08 explicitly confirmed, not assumed | manual (self-driven, Playwright-UAT-authorized policy) | — (interactive; `web-ext run -t firefox-desktop --verbose`) | ❌ Wave 0 | ⬜ pending |
...
- [ ] `extension/vitest.config.ts` — new, no prior test framework exists for `extension/` (Plan 13-02, Task 1)
- [ ] `extension/lib/platform/prf-support.test.ts` — RED-first TDD scaffold (Plan 13-02, Task 1)
- [ ] `extension/playwright.config.ts` + `extension/e2e/dual-browser.spec.ts` — new Chromium E2E harness (Plan 13-03, Task 1)
...
- [ ] `nyquist_compliant: true` set in frontmatter (flip during Plan 13-04, Task 2, once all 21 checklist rows are green)
