...
| 31-03-T3 | 31-03 | 3 | MOD-01 (SC1), MOD-02 (SC2) | — | Per-recipient level + collection-count/destination-id live proof | live e2e (server state) | `cd web && npm run build && npx playwright test e2e/sharing.spec.ts e2e/shared-sync.spec.ts e2e/export-disclosure.spec.ts e2e/family-wide-sharing.spec.ts --retries=0` | ✅ extend | ⬜ pending |
| 31-04-T1 | 31-04 | 4 | MOD-01 | T-31-13 | Pending-revocations honesty summary | unit (TDD) + falsification | `cd web && npm run compile && npm test && npm run build && npx playwright test e2e/sharing.spec.ts e2e/shared-sync.spec.ts e2e/export-disclosure.spec.ts e2e/family-wide-sharing.spec.ts --retries=0` | ❌ new | ⬜ pending |
