...
1. **Does AD-binding `revision` on the payload alone (not the key-wrap) fully satisfy VAULT-02's "revision rollback" protection, or does it need an additional client-side highest-seen-revision check?**
   ...
   - Recommendation: Phase 2's success criterion ("a test mutates the AD and asserts decryption is rejected") is satisfiable and correct as scoped — implement it as specified. Flag the deeper rollback-detection-via-client-side-revision-tracking question for Phase 5 (sync) planning, where the revision-gated pull model is the natural place to add it; note this explicitly in the Phase 2 plan's "out of scope" section so it isn't mistaken for a Phase 2 gap.

2. **Should `folders`/tags membership be encrypted *inside* each item's payload (per CONTEXT.md's "no plaintext folder_id column"), and if so, how does VAULT-03's folder navigation avoid a full-vault decrypt on every folder switch?**
   ...
   - Recommendation: Given VAULT-04 already requires full client-side decrypt-into-memory for search, folder filtering is just another client-side filter predicate over the same already-decrypted in-memory item list — no separate design is needed. Tag storage should follow the same shape (an array of tag IDs/names inside the item payload, tag *definitions* possibly as their own encrypted `folders`-shaped table if tags need to be renamed/colored independently — planner's call, cheap either way since both are client-side-filtered).
