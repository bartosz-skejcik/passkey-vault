...
fixed_count: 17
total_count: 19
last_updated: 2026-08-19T18:40:00.000Z
---
...
| 19 | 31 | deviation | web/src/components/vault/SharingOverviewPanel.tsx | 315 | F-5 (31-VERIFICATION.md gap closure, deliberately NOT fixed there per its own explicit instruction): the Sharing overview's per-person revoke button filters only `family_wide_kind !== "item_bucket"`, so a family-wide FOLDER still renders a live revoke affordance for each of its members. CR-02's own fix (this same phase) added a server-side backstop -- `collections::revoke_access` now refuses on EVERY family-wide collection (`is_family_wide_collection`, folder included), not only `item_bucket` -- so clicking that button on a family-wide folder now 403s and surfaces the generic `share.revokeFailed`, rather than silently self-reverting on the next keyholder unlock as it did pre-CR-02. Probe-confirmed (31-VERIFICATION.md P4): `DELETE .../access/{user}` on a family-wide folder -> 403, row survives -- the BEHAVIOUR is fenced, only the OFFER is stale. `SharingOverviewPanel.tsx` is explicitly out of scope for Phase 31 (`31-CONTEXT.md`: "Out of scope: the Family & Sharing settings surface (Phase 33)"). Fix: extend the same-file filter from `family_wide_kind !== "item_bucket"` to `family_wide_kind === null`, mirroring `ShareDialog.tsx`'s own `editableExistingFolders` filter (CR-02's client-side fix) exactly. | open |  | 2026-08-19T03:00:00.000Z |  |
...
[
  ...
  },
  {
    ...
    "status": "fixed",
    "reason": "Fixed at the pattern level in 64558a0 (Phase 26 WR-08) — create/update/delete + folder ops all contain post-await bookkeeping throws; moveVaultItem (Phase 32) shipped with the same containment. The ledger entry was never updated when the fix landed. Backlog sweep 2026-08-19 verified by falsification (stripping each guard turns its test red) and added the missing regression coverage for updateVaultItem/deleteVaultItem/moveVaultItem in store.test.ts.",
    "recorded_at": "2026-08-06T13:24:13.762Z",
    "resolved_at": "2026-08-19T18:40:00.000Z"
  },
  {
    "id": 12,
    "kind": "stub",
    ...
    "status": "fixed",
    "reason": "Resolved by decision, not by masking: DEBT-02 closed in Phase 29 as 'disclose, not mask' — ExportDialog renders export.hiddenPasswordDisclosure with a hydration-gated reactive count, proven live by export-disclosure.spec.ts reading actual downloaded JSON+CSV bytes. The extension half is vacuously satisfied: the extension has no export surface at all (grep-confirmed; 27-CONTEXT.md deliberately scoped export out per EXT-06).",
  ...
  },
  {
    ...
    "status": "fixed",
    "reason": "Closed by Phase 31: the destination selector lists every edit-level ordinary collection (a half-granted one qualifies — the creator's own edit row is written before any per-member grant), rows seed currentLevel from getCollectionAccessList, and the grant path for an existing destination is reshareCollectionToNewMember (unwrap-own-sealed-key, reseal recovered CK — verbatim the composition this entry predicted). Proven by sharing.spec.ts SC2 live (collection count unchanged) and ShareDialog.real-wasm.test.ts naming WINDOWS #13 directly. Caveat recorded: sidebar shared-folder rows still have no kebab; recovery goes through the dialog's destination selector.",
  ...
  },
  {
    ...
    "resolved_at": "2026-08-11T09:40:00.000Z"
  },
  {
    "id": 16,
    "kind": "deviation",
  ...
  },
  {
    ...
    "kind": "deviation",
    "phase": "30",
    "file": "crates/pv-server/src/routes/families.rs",
    "line": null,
    "description": "Flagged during the WINDOWS #16/#15 investigation, deliberately NOT fixed to avoid scope creep under time pressure: `family_wide_pending`'s `resealable` query has no access-level filter, while the client's reseal flow calls `collections::add_member`, which is RequireEdit-gated. This is the same conceptual bug as the invite-propagation 403 fixed in d07c2a7 (a read-level holder being asked to propagate a grant it cannot legally make), on the lazy-reseal half of FSH-02 instead of the invite-carried half. No currently-failing test exercises it, so it is latent rather than observed. Likely fix: apply the same `require_collection_access_for_propagation` bound, or filter `resealable` by the caller's own access_level.",
    "status": "fixed",
    "reason": "",
  ...
  },
  {
    ...
    "kind": "deviation",
    "phase": "31",
    "file": "web/src/components/vault/SharingOverviewPanel.tsx",
    "line": 315,
    "description": "F-5 (31-VERIFICATION.md gap closure, deliberately NOT fixed there per its own explicit instruction): the Sharing overview's per-person revoke button filters only `family_wide_kind !== \"item_bucket\"`, so a family-wide FOLDER still renders a live revoke affordance for each of its members. CR-02's own fix (this same phase) added a server-side backstop -- `collections::revoke_access` now refuses on EVERY family-wide collection (`is_family_wide_collection`, folder included), not only `item_bucket` -- so clicking that button on a family-wide folder now 403s and surfaces the generic `share.revokeFailed`, rather than silently self-reverting on the next keyholder unlock as it did pre-CR-02. Probe-confirmed (31-VERIFICATION.md P4): `DELETE .../access/{user}` on a family-wide folder -> 403, row survives -- the BEHAVIOUR is fenced, only the OFFER is stale. `SharingOverviewPanel.tsx` is explicitly out of scope for Phase 31 (`31-CONTEXT.md`: \"Out of scope: the Family & Sharing settings surface (Phase 33)\"). Fix: extend the same-file filter from `family_wide_kind !== \"item_bucket\"` to `family_wide_kind === null`, mirroring `ShareDialog.tsx`'s own `editableExistingFolders` filter (CR-02's client-side fix) exactly.",
    "status": "open",
    "reason": "",
