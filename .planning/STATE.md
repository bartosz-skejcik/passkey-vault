...
- [Phase 28] **`web/playwright.config.ts:128`'s `reuseExistingServer: !process.env.CI` silently adopts a stray local `pv-server`.** The config's own DB isolation (a unique `mkdtemp` dir baked into the `webServer` command) is defeated whenever ANYTHING is already listening on `localhost:8620` — Playwright reuses that process and its database instead of building fresh and spinning up its own isolated instance. This is not hypothetical: it happened during 28-02 (recorded there and in 28-02-SUMMARY.md:148-152), writing throwaway `pv-e2e-*` accounts into the developer's REAL `data/pv.db` before it was caught (additive-only, nothing existing touched/deleted) — `data/pv.db` still carries 12 such accounts among 48 users as of this session, confirmed unchanged again during 28-04's own live runs. **Not fixed** (out of scope for a gap-closure pass) — recording per 28-VERIFICATION.md's explicit ask. Two cheap mitigations for a future session: set `reuseExistingServer: false` unconditionally (the webServer command is idempotent and the port is only used by this suite), or move the suite to a dedicated port. Always run `lsof -i :8620` before any live web e2e run and use an isolated server against a throwaway DB — never `data/pv.db` — until this is fixed.
- [Phase 28] **`extension/e2e` requires an externally-started `pv-server` with `PV_STATIC_DIR` set, undocumented anywhere.** Unlike `web/e2e` (which owns its own `webServer` config), the extension suite expects a server the operator starts by hand — and the extension's sign-in ceremony page is served by that server's static handler. Without `PV_STATIC_DIR` pointed at `web/out`, the ceremony tab fails obscurely at `chrome-error://chromewebdata` with no actionable error. This has now cost **two separate agents a full failing run** (28-VERIFICATION.md's own verifier hit it, and it is recorded again here so a third agent does not repeat it). Needed env for a live extension e2e run: `PV_STATIC_DIR=web/out` and `PV_EXTENSION_ORIGINS` set, plus `--retries=0` for a clean single-attempt DB. Not fixed (out of scope) — this is a documentation gap in the extension config/README that a future session should close.
- **CLOSED 2026-08-09 (quick/280809-blf):** v0.4 audit debt #1 — `buildLoginFields()` clobbering the whole `ItemFields` object on a capture-UPDATE — is fixed on the extension side. `confirmUpdateLogin` now calls a new `buildUpdatedLoginFields()` builder that preserves `name`/`notes`/`tags`/`folderId` from the existing item and MERGES `urls` (via `frame-guard.ts`'s now-exported `originEquals`) instead of resetting/truncating them; `buildLoginFields` itself is now CREATE-only. TDD-proven (RED then GREEN, `98de53d`/`bd40d96`); the three Phase 28 refusal gates (`sharedToMe`, non-`edit` collection access) are unchanged, same order, verified via `git diff` before committing. This also resolves the narrower Phase 27 finding recorded above (name-reset on capture-update) as a side effect. **The web app was checked and does NOT share this defect** — `web/src/lib/vault/store.ts`'s `updateVaultItem` has no capture-on-submit analogue; its only caller (`ItemForm.tsx`) always pre-populates the save call with the item's full existing fields, never rebuilds from a narrow subset. See `.planning/quick/280809-blf-capture-update-field-clobber/280809-blf-SUMMARY.md`.
