// This is web's OWN local recreation of packages/pv-ui/i18n/engine.test.ts
// (DS-02, plan 16-04) — SAME test content as that canonical copy and
// extension/lib/i18n/engine.test.ts, mirroring the
// packages/pv-ui/generator/password.test.ts x3 precedent, EXCEPT that
// (unlike that precedent) there is no local web/src/lib/i18n/engine.ts
// shim file this phase — dictionary.ts's own DICTIONARY/t/interpolate stay
// the consumer-facing surface, so this file imports the shared engine
// DIRECTLY from `pv-ui/i18n/engine` so `cd web && npx vitest run` actually
// exercises the real shared module through web's own resolution path.
//
