...
**Trzy znane ograniczenia — zapisane wprost, nie obejście po cichu:**
...
3. **`seal`/`unseal` nie uwierzytelniają żadnej ze stron (WR-10, Phase 21 code review iteration 2).** Anonymous sealed box z definicji nie ma klucza nadawcy — nie ma więc nic, względem czego uwierzytelnić nadawcę. Każdy, kto zna PUBLICZNY klucz recipienta (publiczny z założenia — publikowany przez serwer/katalog członków), może zapieczętować pod niego dowolny plaintext, a recipient go zaakceptuje bez błędu. Symetrycznie: złośliwy/przejęty serwer, który podmieni WŁASNY, w pełni poprawny (nie small-order) klucz publiczny jako "klucz recipienta X", odzyska każdy Collection Key zapieczętowany pod ten podmieniony klucz — walidacja ENCODINGU klucza (poniższy punkt CR-01) nie jest tym samym, co walidacja jego PROVENANCE. Zanim warstwa sharing/invite (Plan 21-06+) zacznie wołać `seal`, `recipient_pk` MUSI być uwierzytelniony poza tą warstwą: podpisany katalog członków, TOFU pin, albo potwierdzenie fingerprintu przez drugą stronę. **CR-01 (rejection małych-rzędów kluczy publicznych)** jest częścią zamrożonego kontraktu tego prymitywu od iteracji 2 code review: `IdentityPublicKey::from_bytes`/`Deserialize` i `unseal`'s `ephemeral_pk` odrzucają 7 znanych small-order encodings X25519 — to zamyka WYŁĄCZNIE wariant ataku niewymagający żadnego klucza (zdegenerowany, publicznie policzalny box key), NIE dostarcza uwierzytelnienia opisanego w tym punkcie.
